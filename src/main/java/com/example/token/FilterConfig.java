...
public class FilterConfig {
    ...
    public FilterRegistrationBean<JwtFilter> filterRegistrationBean(JwtFilter jwtFilter){
        ...
        filterReg.addUrlPatterns("/api/member1/update.do", "/api/member1/delete.do" ,"/api/customer/update.do","/api/customer/delete.do", "/api/seller/update.do", "/api/seller/delete.do","/api/review/update.json"
        ,"/api/review/delete.json", "/api/review/insert.json"
        );
    ...
    // 판매자 정보 변경, 탈퇴, 암호 변경....
    @Bean // 자동으로 객체를 생성하기 위해서
    public FilterRegistrationBean<JwtFilter1> filterRegistrationBean(JwtFilter jwtFilter){
        ...
        //여기가 필터를 통과시킬 URL 설정 부분
        // 전체 한 번에 필터 설정하려면 /api/member1/* (지금은 로그인, 회원가입은 빼야해서 *쓰면 안됨)
        filterReg.addUrlPatterns("/api/member1/update.do", "/api/member1/delete.do");
