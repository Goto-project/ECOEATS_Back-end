...
public class TokenCreate {
    ...
    public Map<String, Object> validateSellerToken(String token) {
        ...
        try {
            ...
            map.put("status", "-1");
            if (e instanceof SignatureException) {
                map.put("message", "JWT Token Invalid Exception");
            } else if (e instanceof ExpiredJwtException) {
                map.put("message", "JWT Token Expired Exception");
            } else {
                map.put("message", "JWT Exception");
            }
    ...
    public Map<String, Object> validateCustomerToken(String token) {
        ...
        try {
            // 검증을 위한 키값 설정 == 발행 시의 키값 (반드시 일치해야 함)
            JwtParser parser = parser().verifyWith(key).build();
            Jws<Claims> result = parser.parseSignedClaims(token); // 토큰 내용 추출
            ...
            if (e instanceof SignatureException) {
                ...
                map.put("message", "JWT Exception: " + e.getMessage());
            }
